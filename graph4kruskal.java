...
public class graph4kruskal {
    ...
    public void addEdge (int v1, eWeight v2){
//        DArray<Edge> tmp;
        int v_1 = v1;
        int v_2 = v2.vertex;
        int w = v2.weight;
    ...
    public void sortingArrayEdges() {
        int in, out;
        for (out = 1; out < sizeEdges; out++) {
            Edge temp = AllEdges.get(out);
            in = out;
            while (in > 0 && AllEdges.get(in - 1).W > temp.W) { // compareToIgnoreCase
...
//            System.out.print(" - " + i);
        ...
        for (int i = 0; i < sizeVertex; i++) {
//            System.out.println("U-" + Unit.get(i));
        }        
    }
...
//        displayArrayEdges();
        ...
        for (int i = 0; i < sizeEdges; i++) {
            if (!Objects.equals(Unit.get(AllEdges.get(i).V1), Unit.get(AllEdges.get(i).V2))){
//                System.out.println("V1--- " + AllEdges.get(i).V1 + " V2--- " + AllEdges.get(i).V2);
                Edges.add(sizeEdgesKruskal, new Edge(AllEdges.get(i).V1, AllEdges.get(i).V2, AllEdges.get(i).W));
                
//                AllEdges.add(sizeEdges, new Edge(v_1, v_2, w));
                sizeEdgesKruskal ++;
//                System.out.println("U1--- " + Unit.get(AllEdges.get(i).V1) + " U2--- " + Unit.get(AllEdges.get(i).V2));
                setUnit(Unit.get(AllEdges.get(i).V1), Unit.get(AllEdges.get(i).V2));
            }
        }
    }
